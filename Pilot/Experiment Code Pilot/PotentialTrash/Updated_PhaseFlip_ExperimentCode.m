...
time.III_JND=[1:0.1:1.5]; % Inter-Interval-Interval for JND
        ...
        if ~ subresults.status.JND_done
            while JND_task
                ...
                while practice_JND
                    ...
                    [JND, ~,resp_eval]=JNDfunction(scr,time,stim,1);
                    % Print evaluation
                    if resp_eval==1
...
%                 io64(scr.triggerPort, scr.triggerPortAddress, 0);
                ...
                [JND, JND_results,~]=JNDfunction(scr,time,stim,0);
...
function [JND,JND_results,resp_eval]=JNDfunction(scr,time,stim,JNDprac)
...
adjustment_steps=0.01; % size of adjustment each step in seconds
maxreversals=18;
...
%     io64(scr.triggerPort, scr.triggerPortAddress,0);
    ...
    if JNDprac
        jndfound=1;
        JND=NaN;
    elseif revers_count==maxreversals % if max reversals has been reached, take the average across the last 10 reversals as the JND
        jndfound=1;
