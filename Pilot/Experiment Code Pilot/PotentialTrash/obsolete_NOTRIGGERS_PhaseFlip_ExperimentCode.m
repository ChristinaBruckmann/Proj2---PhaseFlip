...
% JND Start Trigger
%  io64(triggerPort, triggerPortAddress,stim.triggervectorJND{1,'JND Start'});
%  pause(0.1)
%  io64(triggerPort, triggerPortAddress,0);
...
while ~jndfound
    ...
    for intervals=1:length(interval_lengths)
        ...
        for fixidx=1:time.ITI
            ...
            cnoise=cnoise+1;
%             if fixidx==1
%                 io64(triggerPort, triggerPortAddress,stim.triggervectorJND{intervals,'JND Mask Onset'});
%             elseif fixidx==2
%                 io64(triggerPort, triggerPortAddress,0);
%             end
        end
        ...
        for cueidx=1:time.cuedur
            ...
            ccue1=ccue1+1;
%              if cueidx==1
%                 io64(triggerPort, triggerPortAddress,stim.triggervectorJND{intervals,'JND Cue 1'});
%              elseif cueidx==2
%                 io64(triggerPort, triggerPortAddress,0);
%             end
        end
        ...
        for cueidx=1:time.cuedur
            ...
            ccue2=ccue2+1;
%             if cueidx==1
%                 io64(triggerPort, triggerPortAddress,stim.triggervectorJND{intervals,'JND Cue 2'});
%              elseif cueidx==2
%                 io64(triggerPort, triggerPortAddress,0);
%             end
        end
    ...
    % Query Response
    % io64(triggerPort, triggerPortAddress,stim.triggervectorJND{1,'JND Question'});
    ...
    if  interval_lengths(1)==round(comparison_length/scr.ifi) && comparison_length>standard_length  && double(response)==1 % if (comparison was presented first & is longer) AND (the answer was 1) --> correct
        resp_eval=1;
        %io64(triggerPort, triggerPortAddress,stim.triggervectorJND{1,'JND Response 1 Correct'});
    elseif interval_lengths(2)==round(comparison_length/scr.ifi) && comparison_length>standard_length && double(response)==2 % if (comparison was presented second & is longer) AND (the answer was 2) --> correct
        resp_eval=1;
       % io64(triggerPort, triggerPortAddress,stim.triggervectorJND{1,'JND Response 2 Correct'});
...
% JND Stop Trigger
%  io64(triggerPort, triggerPortAddress,stim.triggervectorJND{1,'JND Stop'});
%  pause(0.1)
%  io64(triggerPort, triggerPortAddress,0);
end
