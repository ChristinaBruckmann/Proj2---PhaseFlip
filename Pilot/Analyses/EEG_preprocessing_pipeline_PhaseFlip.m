...
raw_data_path = 'Y:\el-Christina\PhaseFlip\PF_Pilot\Raw Data\Raw EEG';
...
numdidx=strfind(SDATA.info.fileNames,'Subj');
partnum=str2double(SDATA.info.fileNames{1,1}(numdidx{1}+4:numdidx{1}+5));

removeEXG8=input('remove EGX8? (1=yes): ');
if removeEXG8
    SDATA.data=SDATA.data(:,1:end-1);
    SDATA.info.channel_labels=SDATA.info.channel_labels(1:end-1);
end
...
% stage 1: create bipolar EOG channels
...
% stage 2: remove bad electrodes to not contaminate all components
...
art_margin_samples=round(0.5*art_margin*SDATA.info.sampling_rate/1000);

channelsForRDI=SDATA.metadata.good_electrodes(SDATA.metadata.good_electrodes<65);
lpf_data_for_RDI = LPF(double(SDATA.data),SDATA.info.sampling_rate,lpf_cutoff_for_rdi,3);
...
% 
% channel_labels_RDI=[SDATA.info.channel_labels(channelsForRDI); {'VEOG'}; {'HEOG'}];
% amp_art_inspected = multichanplot([SDATA.data(:,channelsForRDI) VEOG HEOG],10,'srate',SDATA.info.sampling_rate,'markdata',amp_art,'channelnames',channel_labels_RDI, 'ylim', [-100 100]);
% 
...
% New version (shows which one is bad)
